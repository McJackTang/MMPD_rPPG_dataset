...
def signal_predict(config, data_loader, method_name):
    ...
    header_list = ['train_dataset', 'Motion', 'Skin_color', 'MAE', 'RMSE', 'MAPE', 'Pearson', 'SNR']
    data_dict = {'train_dataset': 'SIGNAL_SIMPLE' + method_name,
                    'Motion': 'S',
                    'Skin_color': 3,
                    'MAE': MAE_PEAK, 
                    'RMSE': RMSE_PEAK, 
                    'MAPE': MAPE_PEAK, 
                    'Pearson': Pearson_PEAK[0][1],}
    with open('/data/rPPG-Toolbox/ROUND_final/test_result_paper.csv', mode='a', encoding='utf-8-sig', newline='') as f:
        f.writelines('\n')
