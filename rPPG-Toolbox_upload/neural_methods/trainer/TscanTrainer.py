...
from neural_methods.loss.TALOS import TALOS
...
class TscanTrainer(BaseTrainer):
    ...
    def __init__(self, config):
        ...
        ### added
        self.train_dataset = config.TRAIN.MODEL_FILE_NAME
        self.Motion = config.TEST.DATA.INFO.MOTION[0]
        self.Skin_color = config.TEST.DATA.INFO.SKIN_COLOR[0]
    ...
    def train(self, data_loader):
        ...
        plt.plot(total_loss)
        plt.show()
        # plt.savefig('/data/rPPG-Toolbox/ROUND_final/'+self.model_file_name+'_train.jpg')  # train的loss
        plt.figure()
        plt.xlabel('epochs')
        plt.ylabel('loss')
        plt.plot(valid_total_loss)
        plt.show()
        # plt.savefig('/data/rPPG-Toolbox/ROUND_final/'+self.model_file_name+'_valid.jpg')  # valid的loss
        print("best trained epoch:{}, min_val_loss:{}".format(self.best_epoch, min_valid_loss))

        # header_list = ['model_file_name', 'best_trained_epoch', 'min_valid_loss', 'MAE', 'RMSE', 'MAPE', 'Pearson', 'best_model']
        # data_dict = {'model_file_name': self.model_file_name,
        #              'best_trained_epoch': self.best_epoch,
        #              'min_valid_loss': min_valid_loss,}
        # with open('/data/rPPG-Toolbox/ROUND3/test_result_1.1.csv', mode='a', encoding='utf-8-sig') as f:
        #     f.writelines('\n')
