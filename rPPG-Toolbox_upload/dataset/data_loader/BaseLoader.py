...
class BaseLoader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        split_idx = item_path_filename.index('_', -12)
    ...
    def preprocess(self, frames, bvps, config_preprocess, large_box=False):
        ...
        for data_type in config_preprocess.DATA_TYPE:
            ...
            elif data_type == "PhysNormalized":
                data.append(BaseLoader.normalized_data(f_c))
            else:
        ...
        elif config_preprocess.LABEL_TYPE == "PhysNormalized":
            bvps = BaseLoader.normalized_label(bvps)
        else:
    ...
    def facial_detection_cv2(self, frame, larger_box=False, larger_box_size=1.0):
        """Conducts face detection on a single frame.
        Sets larger_box=True for larger bounding box, e.g. moving trials."""
        detector = cv2.CascadeClassifier(
            '/data/rPPG-Toolbox/dataset/haarcascade_frontalface_default.xml')
    ...
    #frames padding 0 in the edge to get large box

    def frames_padding(self,frames,new_image_width,new_image_height):
       color = (0,0,0)
       
       resize_frames = np.full((len(frames),new_image_height,new_image_width, 3), color, dtype=np.uint8)
       for i in range(len(frames)):
              ...
              # save result
              # cv2.imwrite("/data/rPPG-Toolbox/result2/lena_centered.jpg", result)
              resize_frames[i] = result
       return resize_frames

    #facial_detection_dlib
    def facial_detection(self, image, larger_box=False, larger_box_size=1.0):
        ...
        if len(dets) < 1:
            print("ERROR:No Face Detected")
            result = [0, 0, image.shape[0], image.shape[1]]
        else:
            det = dets[0] #get the biggest face
            if len(dets)>1:
                print("WARN:More than one faces are detected(Only cropping the biggest one.)")
                my_img = cv2.rectangle(image, (det.left(), det.top()), (det.right(), det.bottom()), (0, 255, 0), 2)                
                cv2.imwrite("/data/rPPG-Toolbox/result2/image_face.png", image)

        # print(det)
        
            result_ltrb = [[det.left(), det.top(), det.right(), det.bottom()]]
            result = [det.left(), det.top(), det.right()-det.left(), det.bottom()-det.top()]
        if larger_box:
            print("Larger Bounding Box")
            result[0] = max(0, result[0] - (larger_box_size - 1.0) / 2 * result[2])
            result[1] = max(0, result[1] - (larger_box_size - 1.0) / 2 * result[3])
    ...
    def resize(self, frames, dynamic_det, det_length,
        ...
        #for TANG dataset
        if self.name == 'TANG':
            frames = self.frames_padding(frames,400,400)
        # print(frames.shape) 
    ...
    def diff_normalize_label(label):
        ...
        # print('before',normalized_label.shape)
        normalized_label = np.append(normalized_label,np.zeros((1)),axis=0)
        # print('after',normalized_label.shape)
    ...
    def normalized_label(label):
        label = 2*(label-np.min(label))/(np.max(label)-np.min(label))-1
        return label
    ...
    #     mean_data = 0.5*(max_data+min_data)
    #     data = data - mean_data
    #     data = data / (0.5*(max_data-min_data))
    #     data[np.isnan(data)] = 0
    #     return data
    ...
    #     mean_label = 0.5*(max_label+min_label)
    #     label = label - mean_label
    #     label= label / (0.5*(max_label-min_label))
    #     label[np.isnan(label)] = 0
    #     return label
